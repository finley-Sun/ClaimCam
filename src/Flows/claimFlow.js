...
export class ClaimFlow {
  ...
  async _mockGenerate(obj) {
    ...
    const targetObj = this._getObjects().find(o => o.id === this.draft.objectId);
    const isDuplicate = targetObj &&
      targetObj.claims &&
      targetObj.claims.some(c => c.policyType === this.draft.policyType);
    ...
    if (isDuplicate) {
      footer.innerHTML = `
        <div class="cf-duplicate-warning">
          A ${this.draft.policyType} claim already exists for this object.
        </div>
        <button class="cf-btn-secondary" id="cf-close-done">Close</button>
      `;
    } else {
      footer.innerHTML = `
    ...
    if (!isDuplicate) {
      ...
      footer.querySelector('#cf-finish').addEventListener('click', () => {
        if (saved) return;
        saved = true;
        footer.querySelector('#cf-finish').disabled = true;
        if (this.onComplete) this.onComplete(claim);
        this.close();
      });
    }
  }
