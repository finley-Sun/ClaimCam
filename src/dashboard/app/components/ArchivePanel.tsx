...
export function ArchivePanel({
  ...
  const totalDocumented = allRooms.flatMap((r) => r.items).reduce((s, i) => s + i.replacementValue, 0);
        ...
        >
          <div className="flex h-full w-[416px] flex-col">
            ...
            <div className="flex shrink-0 items-center justify-between px-4 py-4">
              <h2 className="text-2xl font-medium text-white" style={{ fontFamily: "var(--font-body)" }}>
                Archive
              </h2>
              <button
                onClick={onClose}
              ...
              <div className="flex flex-col gap-4 px-4 pb-8">
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-xs text-[#8695A7]">Currently viewing</p>
                      <h3 className="text-lg font-semibold text-white">{room.name}</h3>
                    </div>
                    <button
                      onClick={() =>
                        toast(
                          belongingsTab === "structure" ? "Document structure" : "Add to this scene",
                          {
                            description:
                              belongingsTab === "structure"
                                ? `Upload blueprints, permits, or invoices for ${room.name}.`
                                : `Capture a new item or detail in ${room.name}.`,
                          },
                        )
                      }
                    ...
                    >
                      ...
                      {belongingsTab === "structure" ? "Document structure" : "Add item"}
                    </button>
                  </div>
                  ...
                  {userType === "tenant" ? (
                    ...
                    /* Homeowner interior: segmented toggle matching Figma design */
                    <div className="flex flex-col gap-3">
                      ...
                      {belongingsTab === "belongings" ? itemList : (
                        <StructureList records={room.structure} sceneName={room.name} />
                      )}
                    </div>
                  )}
